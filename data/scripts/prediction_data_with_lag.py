...
def prepare_sales(file):
    ...
    sales_info['Year'] = sales_info['Year'] + 0
    ...
    sales_info = sales_info[(sales_info['Year'] >= 2017) & (sales_info['Year'] <= 2018)]

    sales_small = sales_small.drop(columns=['sale_price', 'actual_sf'])
    sales_small = sales_small[(sales_small['Year'] >= 2017) & (sales_small['Year'] <= 2018)]
...
def prepare_info(file):
    ...
    info_small = info_small[(info_small['Year'] >= 2017) & (info_small['Year'] <= 2018)]
...
if __name__ == "__main__":
    ...
    df.to_csv('../prediction_data_with_lag.csv', index=False)
